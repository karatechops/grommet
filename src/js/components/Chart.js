...
import Chart, { Base, Layers, Line, Bar, Area, Threshold, HotSpots, Axis }
  from './chart/Chart';
...
export default class ChartV1 extends Component {
  ...
  render () {
    const { series, size, type, min, points, xAxis } = this.props;
    ...
    if (! max) {
      ...
      series.forEach(serie => {
        serie.values.forEach(value => {
          value = (Array.isArray(value) ? value[1] : value.y);
    ...
    if (this.props.threshold) {
      thresholds = (
        <Threshold min={min} max={max} value={this.props.threshold} />
      );
    }

    let hotspots;
    const graphs = series.map((serie, seriesIndex) => {
      const values = serie.values.map((value) => {
        if (Array.isArray(value)) {
          return value[1];
        } else {
          if (value.onClick && ! hotspots) {
            hotspots = (
              <HotSpots count={serie.values.length} onClick={this._onClick}
      ...
      return (
        <Graph key={seriesIndex} min={min} max={max} values={values.reverse()}
          points={points}
          colorIndex={serie.colorIndex} activeIndex={this.state.index} />
      );
    });

    let topAxis, bottomAxis;
    if (xAxis) {
      const values = (Array.isArray(xAxis) ? xAxis : xAxis.data).map((item, index) => (
        { label: item.label, value: index }
      ));
      const axis = <Axis min={min} max={max} values={values} />;
      if ('bottom' === xAxis.placement) {
        bottomAxis = axis;
      } else {
        topAxis = axis;
      }
...
ChartV1.contextTypes = {
  intl: PropTypes.object
};

ChartV1.defaultProps = {
